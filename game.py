import PySimpleGUI as sg
import datetime
import math
from vectors import *
from pysubsim import *

sg.theme('BlueMono')   # Add a touch of color
sg.set_options(suppress_raise_key_errors=False,
               suppress_error_popups=False, suppress_key_guessing=False)

# All the stuff inside your window.

drag_coefficient = 0.07  # drag coefficient of the submarine in water
LENGTH_688 = 110  # meters
RADIUS_688 = 2  # meters
MASS_688 = 6927  # about 7 tons


class World(object):

    def __init__(self):
        # max 120 degrees per minute, 360 in 3 minutes
        self.MAX_TURN_RATE_HOUR = math.radians(120) * 60
        self.MAX_DEEP_RATE_FEET = 1  # 1 foot per second
        self.MAX_SPEED = 36.0  # Knots or nautical mile per hour
        # acceleration in knots/hour^2 -> imax acceleration 2 Knots / second^2
        self.MAX_ACCELERATION = 0.1 * 3600

        # turbines 35,000 hp (26 MW), 1 auxiliary motor 325 hp (242 kW)
        self.turbine_level = 50.0  # from -100% to +100%
        # self.heading: float = 0.0
        self.drag_length:float = 0.0# newtons
        self.drag: Point2D = Point2D(0, 0) # newtons
        # the angle of the ship is rotated in relation to north, in radians
        self.ship_bearing = 0.0
        self.acceleration_angle: float = 0.0
        self.acceleration: Point2D = Point2D(0, 0)
        self.speed: Point2D = Point2D(0, 0)  # knots
        self.speed_length: float = 0.0 # knots
        self.speed_angle: float = 0.0
        self.position: Point2D = Point2D(0, 0)  # miles
        self.time = datetime.datetime(2022, 1, 1, 0, 0, 0)

        # moving_direction: the direction the ship is moving in relation to north, in radians
        self.speed_angle, self.speed_length = self.speed.angle_length()

        self.rudder = 0  # rudder in radians per minute

    def turn(self, time_elapsed_in_hours: float):
        self.time = self.time + \
            datetime.timedelta(seconds=time_elapsed_in_hours*3600)
        self.turbine_acc = self.MAX_ACCELERATION * self.turbine_level / 100
#         self.turbine_acc_x, self.turbine_acc_y = project_xy(world.ship_bearing, self.turbine_acc)

        if self.rudder != 0:
            # diff is the difference between the angle the sub is mov ing and the angle of the ship is bearing
            # meaning the ship the turning left or right
            diff = (self.rudder * time_elapsed_in_hours * self.speed_length)
            self.acceleration_angle = self.ship_bearing - diff
            self.ship_bearing += diff
        else:
            self.acceleration_angle = self.ship_bearing

        #     diff = self.course - world.ship_bearing

        #     # correction if the drag factor since the sub is making a turn
        # * (1 + abs(500 * math.sin(diff)))
        # self.drag_factor = self.DRAG_FACTOR

        # drag force
        # self.drag_force = self.drag_factor * (self.speed_ ** 2)
        self.drag_length = calculate_drag_force(
            drag_coefficient, RADIUS_688, LENGTH_688, self.speed_length, 0, self.ship_bearing - self.speed_angle)
        # drags accelerates against the current movement
        self.drag = project_xy_to_point(
            self.speed_angle, -1.0 * self.drag_length)
        
        # calculates all forces that actuates in the ship

        # turbine accelerates in direction the ship in bearing
        self.acceleration = project_xy_to_point(
            self.acceleration_angle, self.turbine_acc)

        # updates speed and position
        self.speed = self.speed + \
            (self.acceleration + self.drag) * time_elapsed_in_hours
        self.speed_angle, self.speed_length = self.speed.angle_length()
        self.position += self.speed * time_elapsed_in_hours

    def debug_pos(self):
        return f'{self.time.strftime("%H:%m:%S")} t_level: {self.turbine_level:0.2f} t_acc:{self.turbine_acc:0.2f} drag:{self.drag:0.2f}({self.drag_x:0.2f},{self.drag_y:0.2f}) acc:{self.acceleration:0.2f} speed:{self.speed_:0.2f} pos:({self.position.x:0.2f},{self.position.y:0.2f})'


world = World()

# col1 = sg.Column([
#     # Categories sg.Frame
#     [sg.Frame('Categories:', [[sg.Radio('Websites', 'radio1', default=True, key='-WEBSITES-', size=(10, 1)),
#                                sg.Radio('Software', 'radio1', key='-SOFTWARE-',  size=(10, 1))]], size=(255, 45 ))],
#     # Information sg.Frame
#     [sg.Frame('Information:', [[sg.Text(), sg.Column([[sg.Text('Account:')],
#                                                       [sg.Input(
#                                                           key='-ACCOUNT-IN-', size=(19, 1))],
#                                                       [sg.Text('User Id:')],
#                                                       [sg.Input(key='-USERID-IN-', size=(19, 1)),
#                                                        sg.Button('Copy', key='-USERID-')],
#                                                       [sg.Text('Password:')],
#                                                       [sg.Input(key='-PW-IN-', size=(19, 1)),
#                                                        sg.Button('Copy', key='-PASS-')],
#                                                       [sg.Text('Location:')],
#                                                       [sg.Input(key='-LOC-IN-', size=(19, 1)),
#                                                        sg.Button('Copy', key='-LOC-')],
#                                                       [sg.Text('Notes:')],
#                                                       [sg.Multiline(
#                                                           key='-NOTES-', size=(25, 5))],
#                                                       ], size=(235, 350), pad=(0, 0))]])], ], pad=(0, 0))


# col2 = sg.Column(
#     [[sg.Frame('Accounts:',
#         [[sg.Column(
#             [[
#                 sg.Input(key='-FF-', size=(22, 1)),

#             ]], size=(150, 400))
#         ]])
#     ]], pad=(0, 0))

# col3 = sg.Column([[sg.Frame('Actions:',
#                             [[sg.Column([[sg.Button('Save'), sg.Button('Clear'), sg.Button('Delete'), ]],
#                                         size=(450, 45), pad=(0, 0))]])]], pad=(0, 0))

col1 = sg.Column([[
    sg.Frame('Navigation:',
             [[sg.Text('Turbine:'), sg.Slider(range=(-100, 100), default_value=0, size=(20, 15), orientation='horizontal', key='-turbine-')],
              [sg.Text('Rudder:'), sg.Slider(range=(-15, 15), default_value=0,
                                             size=(20, 15), orientation='horizontal', key='-rudder-')],
              [sg.Text('Turbine Acceleration:'), sg.Text(
                  '0.0', key='-turbine_acc-')],
              [sg.Text('Rudder:'), sg.Text('0.0', key='-rudder_read-')],
              [sg.Text('Drag:'), sg.Text('0.0', key='-drag-')],
              [sg.Text('Acceleration:'), sg.Text('0.0', key='-acc-')],
              [sg.Text('Acceleration angle:'), sg.Text(
                  '0.0', key='-acc_angle-')],
              [sg.Text('Speed:'), sg.Text('0.0', key='-speed-')],
              [sg.Text('Position:'), sg.Text('0.0', key='-position-')],

              [sg.Text('Moving Direction:'), sg.Text(
                  '0.0', key='-moving_direction-'),sg.Text('diff:'), sg.Text(
                  '0.0', key='-moving_direction_diff-')],
              [sg.Text('Ship Bearing:'), sg.Text('0.0', key='-ship_bearing-')],
              ])
]])
col2 = sg.Column([[sg.Text('Speed:'), sg.Text('0.0', key='-speed-')],
                 [sg.Canvas(size=(100, 100), background_color='white', key='-canvas-')]])

# The final layout is a simple one
layout = [[col1, col2]]

# A perhaps better layout would have been to use the vtop layout helpful function.
# This would allow the col2 column to have a different height and still be top aligned
# layout = [sg.vtop([col1, col2]),
#           [col3]]

# Create the Window
window = sg.Window('PySubSim', layout, finalize=True)

cir = window['-canvas-'].TKCanvas.create_oval(50, 50, 100, 100)

# Event Loop to process "events" and get the "values" of the inputs
last_post_read_time = datetime.datetime.now()
while True:
    # pre_read_time = datetime.datetime.now()
    # processing_time = (pre_read_time - last_post_read_time).total_seconds()
    # time_to_sleep = sleep_time - int(processing_time*1000)
    # time_to_sleep = max(time_to_sleep, 0)

    event, values = window.read(100)
    now = datetime.datetime.now()
    delta = (now-last_post_read_time).total_seconds()
    print(delta)
    last_post_read_time = now
    print(event, values)
    # if user closes window or clicks cancel
    if event in (sg.WIN_CLOSED, 'Cancel') and sg.popup_yes_no('Do you really want to exit?') == 'Yes':
        break
    elif event == 'Save':
        print('Save')

    world.turbine_level = values['-turbine-']
    world.rudder = math.radians(values['-rudder-'])
    world.turn(1/3600)

    window['-turbine_acc-'].update(f'{world.turbine_acc:0.2f}')
    window['-rudder_read-'].update(f'{world.rudder:0.2f}')
    window['-acc-'].update(
        f'{world.acceleration.length():0.2f}({world.acceleration.x:0.2f},{world.acceleration.y:0.2f})')
    window['-acc_angle-'].update(
        f'{math.degrees(world.acceleration_angle):0.2f}')
    window['-drag-'].update(
        f'{world.drag.length():0.2f}({world.drag.x:0.2f},{world.drag.y:0.2f})')
    window['-speed-'].update(
        f'{world.speed.length():0.2f}({world.speed.x:0.2f},{world.speed.y:0.2f})')
    window['-position-'].update(
        f'({world.position.x:0.2f},{world.position.y:0.2f})')

    window['-moving_direction-'].update(
        f'{math.degrees(world.speed_angle):0.2f}')
    window['-moving_direction_diff-'].update(
        f'{math.degrees(world.speed_angle - world.ship_bearing):0.2f}')
    window['-ship_bearing-'].update(f'{math.degrees(world.ship_bearing):0.2f}')

    # canvas = window['-canvas-'].TKCanvas.create_oval(10, 10, 20, 100)


window.close()


###################################

# window = sg.Window('Graph test', layout, finalize=True)

# graph = window['graph']         # type: sg.Graph
# circle = graph.draw_circle((75, 75), 25, fill_color='black', line_color='white')
# point = graph.draw_point((75, 75), 10, color='green')
# oval = graph.draw_oval((25, 300), (100, 280), fill_color='purple', line_color='purple')
# rectangle = graph.draw_rectangle((25, 300), (100, 280), line_color='purple')
# line = graph.draw_line((0, 0), (100, 100))
# arc = graph.draw_arc((0, 0), (400, 400), 160, 10, style='arc', arc_color='blue')
# poly = graph.draw_polygon(((10,10), (20,0), (40,200), (10,10)), fill_color='green')
# while True:
#     event, values = window.read()
#     print(event, values)
#     if event == sg.WIN_CLOSED:
#         break
#     if event in ('Blue', 'Red'):
#         graph.TKCanvas.itemconfig(circle, fill=event)
#     elif event == 'Move':
#         graph.MoveFigure(point, 10, 10)
#         graph.MoveFigure(circle, 10, 10)
#         graph.MoveFigure(oval, 10, 10)
#         graph.MoveFigure(rectangle, 10, 10)
#         graph.MoveFigure(arc, 10, 10)
#         graph.MoveFigure(poly, 10, 10)

#########################

# def main():
#     layout = [[sg.Text('A toggle button example')],
#               [sg.Text('A graphical version'),
#                sg.Button('', image_data=toggle_btn_off, key='-TOGGLE-GRAPHIC-', button_color=(sg.theme_background_color(), sg.theme_background_color()), border_width=0)],
#              [sg.Button('On', size=(3, 1), button_color='white on green', key='-B-'),  sg.Button('Exit')]]

#     window = sg.Window('Toggle Button Example', layout)

#     down = graphic_off = True
#     while True:             # Event Loop
#         event, values = window.read()
#         print(event, values)
#         if event in (sg.WIN_CLOSED, 'Exit'):
#             break
#         elif event == '-B-':                # if the normal button that changes color and text
#             down = not down
#             window['-B-'].update(text='On' if down else 'Off', button_color='white on green' if down else 'white on red')
#         elif event == '-TOGGLE-GRAPHIC-':   # if the graphical button that changes images
#             graphic_off = not graphic_off
#             window['-TOGGLE-GRAPHIC-'].update(image_data=toggle_btn_off if graphic_off else toggle_btn_on)

#     window.close()

# if __name__ == '__main__':
#     toggle_btn_off = b''
#     toggle_btn_on = b''
#     main()

##########

# def LEDIndicator(key=None, radius=30):
#     return sg.Graph(canvas_size=(radius, radius),
#              graph_bottom_left=(-radius, -radius),
#              graph_top_right=(radius, radius),
#              pad=(0, 0), key=key)

# def SetLED(window, key, color):
#     graph = window[key]
#     graph.erase()
#     graph.draw_circle((0, 0), 12, fill_color=color, line_color=color)


# layout = [[sg.Text('My LED Status Indicators', size=(20,1))],
#           [sg.Text('CPU Use'), LEDIndicator('_cpu_')],
#           [sg.Text('RAM'), LEDIndicator('_ram_')],
#           [sg.Text('Temperature'), LEDIndicator('_temp_')],
#           [sg.Text('Server 1'), LEDIndicator('_server1_')],
#           [sg.Button('Exit')]]

# window = sg.Window('My new window', layout, default_element_size=(12, 1), auto_size_text=False, finalize=True)

# i = 0
# while True:  # Event Loop
#     event, value = window.read(timeout=400)
#     if event == 'Exit' or event == sg.WIN_CLOSED:
#         break
#     if value is None:
#         break
#     i += 1
#     SetLED(window, '_cpu_', 'green' if random.randint(1, 10) > 5 else 'red')
#     SetLED(window, '_ram_', 'green' if random.randint(1, 10) > 5 else 'red')
#     SetLED(window, '_temp_', 'green' if random.randint(1, 10) > 5 else 'red')
#     SetLED(window, '_server1_', 'green' if random.randint(1, 10) > 5 else 'red')
# window.close()
